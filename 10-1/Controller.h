...
* BGM은 예를 들어 "C:\\Users\\Desktop\\tetris\음악명" 경로를 문자열로 저장합니다.
* 경로는 \가 아니라 \\를 해야함.
* #define문으로 BGM 정의하고 main함수에서 사용 가능.
* main()문에 BGM 부분 경로를 적성 해도 됨.
*/
#include<mmsystem.h>
#pragma comment(lib, "winmm.lib")
//음원은 저작권 문제로 따로 제공하지 않습니다.
//#define BGM "C:\\Users\\Desktop\\tetros\\음악명.wav" / WAV Full Path
...
enum blockProperity {
	BlockSIZE = 4, //블록의 크기
	BlockRotate = 4, //볼록의 회전
	BlockType = 7, //블록의 종류
	BlockStartX = 14, //블록의 커서창에서 시작될 x좌표
	BlockStartY = 4 //블록의 커서창에서 시작될 y좌표 
};
	...
	*/
	...
	/**
	* 회전 기능의 방향키를 입력 받을 시
	* turm(현재 블록의 회전 상태 0,1,2,3중 예를 들어 turn == 0 번째 회전 상태(초기블럭이라면)
	*'->' 방향키를 통해 turn ==1이 되고, 따라서
	*block에서 정의한
	...
	*/
	short turn;
	void RotateBlock(); //방향키 받을 시 rotate를 통해 방향키 번경
	...
	* bottom에서 블록이 닿아 고정된 블록과 닿으면, true 반환
	* 그럴 경우 현재 좌표값이 보드의 바닥과 닿거나, 바닥화?된(고착화된?) 블록이라고 판단되기에
	* 현재 좌표에 출력된 블록을 블록의 바닥과 인식시킵니다. blockFixed(int shape,int rotate)
	*/
	...
	*/
	...
	//미리보기 블록 만들고 지울 떄 해당 좌표 저장해야합니다.
	COORD previewPoint;
